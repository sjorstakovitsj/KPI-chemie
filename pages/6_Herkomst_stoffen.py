...
# DATA VOORBEREIDING
# ---------------------------------------------------------
st.info("Hieronder worden de gegevens getoond op basis van de **jaren en perioden geselecteerd in de zijbalk**.")

# We gebruiken df_filtered (al gefilterd op jaar en periode in de sidebar)
df_herkomst = df_filtered.dropna(subset=['Stof']).copy()

# Vectorized check voor detecties (sneller dan string contains in loop)
# Het Parquet-buildproces heeft 'Limietsymbool' al opgeschoond (geen NaN)
...
with col_total_all:
    ...
    st.plotly_chart(fig_pie_total, width='stretch')
    ...
    st.dataframe(top_stoffen, width='stretch')
...
with col_total_det:
    ...
    if not df_detecties.empty:
        ...
        st.plotly_chart(fig_pie_det, width='stretch')
        ...
        st.dataframe(top_stoffen_det, width='stretch')
    else:
...
st.markdown("De grafiek toont data uit **alle jaren**, ongeacht het jaarfilter in de zijbalk, maar houdt wel rekening met het geselecteerde seizoen of halfjaar.")
...
with col_trend_sel2:
    ...
    all_meetpunten_trend = sorted(filter_options["meetpunten"])
...
if selected_meetpunten_trend:
    # Deze sectie gebruikt bewust alle jaren. Periode- en meetpuntfilters worden
    # door DuckDB uitgevoerd; alleen de optionele rapportagegrensfilter blijft lokaal.
    df_trend = query_data(
        periodes=tuple(geselecteerde_periodes),
        meetpunten=tuple(selected_meetpunten_trend),
        kolommen=("Datum", "Meetpunt", "Stofgroep", "Limietsymbool"),
    )

    if alleen_detecties_trend:
        mask_det_main = ~df_trend['Limietsymbool'].astype(str).str.contains('<', na=False)
        df_trend = df_trend[mask_det_main].copy()
...
if meetpunten_list:
    ...
    if not df_mp_all.empty:
        ...
        with col_mp_1:
            ...
            st.plotly_chart(px.pie(counts_all, values='Aantal', names='Stofgroep'), width='stretch')
        ...
        with col_mp_2:
            ...
            if not df_mp_det.empty:
                ...
                st.plotly_chart(px.pie(counts_det, values='Aantal', names='Stofgroep'), width='stretch')
            else:
        ...
        if not df_table_source.empty:
            ...
            if beschikbare_groepen:
                ...
                # 6. Weergeven
                st.dataframe(table_agg, width='stretch')
            else:
        ...
        # Tijdens de Parquet-build krijgen niet-gematchte stoffen het label 'Onbekend'
        ...
        if not df_onbekend.empty:
            ...
            st.dataframe(onbekend_summary, width='stretch')
        else:
            ...
            if not df_table_source.empty:
                ...
                if beschikbare_groepen:
                    ...
                    st.dataframe(detail_agg, width='stretch')
                else:
